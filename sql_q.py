...
class HTMLTable(list):
    ...
    def _repr_html_(self):
        ...
        # tbody += '<tr style="height:50px;">' + ''.join([('<td></td>') for r in row]) + '</tr>'
        thead += '</tr></thead>'
        tbody += '</tbody>'
        table += thead + tbody
        return table
    ...
    def display(self, columns=[], msg=None):
        table_str = HTMLTable([columns] + self.data, self.id_)._repr_html_()
        table_str = table_str.replace('<table', '<table class="table-striped table-hover"')
        display(
            Javascript(
                """
...
# @timer
def _SQL_Q(path, cell, thread_parent=None):
    """
...
def sql_q(path, cell):
    thread_parent = sys.stdout.parent_header
    q = new_queue()
    t = threading.Thread(target=_SQL_Q, args=(path, cell, thread_parent))
    t.daemon = True
    t.start()
    return None
